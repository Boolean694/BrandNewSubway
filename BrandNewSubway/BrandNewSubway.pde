  ...
  if (mouseX >= width * 0.65) { //clicked in menu
    ...
    if (useless) {//click on station
      if (!makingTransfers) {
        ...
        if(!(currentToggle == null || selected.lines.contains(currentToggle))) { //if not transferring lines
          selected.lines.add(currentToggle);
          if(currentToggle.stations.size() == 0 || currentToggle.stations.size() == 1) { //if first or second stn on line
            currentToggle.stations.add(selected);
          }
          else {
            transferLines(selected);
          }
        }
      }
      else { 
...
void transferLines(Station adding) {
  int stind = 0;
